...
jDate = year_romaji + "年(とし) " + month_romaji + " " + day_romaji + " " + weekday_romaji
# 按年、月、日、星期输出日期的平假名，部分我还没练熟的汉字后面会加上平假名注释。
print(jDate)

# 输出日期的年、月、日、星期，按前面转换的平假名长度，尽量对其年、日、星期的输出。
str_year = ""
...
# 给出自行朗读的时间，倒计时后，调用有道API朗读。在は、年等字的发音不准确。
countdownTimer = 8
print("对比音频倒数%D秒", countdownTimer)
i = 1
while i < countdownTimer:
	print(countdownTimer-i)
	i+=1
	time.sleep(1)
print(jDate)
import ttsYoudao
from playsound import playsound
playsound(ttsYoudao.connect(jDate))
print("你读对了么？")